...
def open_experiment(exp_folder_name, start_file, end_file, file_name='atmos_monthly.nc', nbits=4, base_dir=None):
    ...
    ds = xr.open_mfdataset(files, decode_times=False) #, autoclose=True) # not used in py3

    return ds

def get_ds_arr_from_exps(exp_folder_names, start_file, end_file, 
        file_name='atmos_monthly.nc', nbits=4, base_dir=None):
    ...
    for exp_folder_nm in exp_folder_names:
        ds = open_experiment(exp_folder_nm, start_file, end_file, 
            file_name=file_name,nbits=nbits, base_dir=base_dir)
        ds_arr.append(ds)
    return ds_arr
